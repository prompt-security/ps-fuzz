...
def setup_logging(logging_level):
    # Configure root logger. Use empty handlers to prevent default (console) output.
    logging.basicConfig(format='%(asctime)s [%(levelname)s] [%(filename)s:%(lineno)d]: %(message)s', level=logging.WARNING, handlers=[])

    logging.getLogger('_client').setLevel(logging_level)
    for module_name in ('prompt_injection_fuzzer', 'chat_clients', 'interactive_chat', 'dynamic_test'):
        logging.getLogger(module_name).setLevel(logging_level)
    ...
    logging.getLogger('').addHandler(file_handler)  # add the file_handler to the root logger
